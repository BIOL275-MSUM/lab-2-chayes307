...
And finally, I can see the 95% confidence interval ranges from a minimum of 4.203409 to a maximum of 5.430491
mean(k) + 1.96 * sem
mean(k) - 1.96 * sem 
...
And finally, I can see the 95% confidence interval ranges from a minimum of 4.203409 to a maximum of 5.43051
...
# plot chick weights
ggplot(chick_data, aes(k = diet, k = weight, color = diet, fill = diet)) +
  geom_jitter(size = 5, shape = 21, alpha = 0.5, width = 0.1) +
  geom_crossbar(
    mapping = aes(kmin = lower, kmax = upper, k = mean), 
    data = chick_summary,
    width = 0.2, alpha = 0.2
  ) +
  labs(
